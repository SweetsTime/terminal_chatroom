...
/* ***********************************函数申明************************************* */
//功能发送函数部分
void emoji(char* name, int fd); //表情包功能 
void listMember(int fd); //输出成员列表功能 
void send_file(char* name, int fd);//tcp传文件 
void download(char* name, int fd);//tcp下载文件 
void send_to(char *name, int fd);//私发消息 
void message(char *name, int fd);//查看聊天记录
void object_detection(char* name, int fd, char *file);//目标识别
void func(int fd); //查看功能
void write_blog(char *name, int fd); //写博客 
void read_blog(char *name, int fd); //读博客
//...其余待开发
...
{
    char* emo_value[7] = {"  ⌯•ω<⌯ಣ", "  ⌯ᵔ⤙ᵔ⌯ಣ","  (๑ò ω ó๑)","  (*/ω＼*)","  QAQ","  >_<","  ╰_╯"};
    char choiceList[100];
    char choice[7];
    char emojiMsg[100];
    char errorMsg[50];
...
void write_blog(char *name, int fd){
    ...
    send(fd, "欢迎来到博客写作，请给您的博客起一个标题吧：\n",
            sizeof("欢迎来到博客写作，请给您的博客起一个标题吧：\n"), 0);
...
int main(){
    ...
    strcat(content, "Hello Vivian\n");
    strcat(content, "Hello Nanako\n");
    strcat(content, "Hello Alice\n");
    strcat(content, "Hello Rosemary\n");
